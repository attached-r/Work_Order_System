...
| GET | `/user/page` | `user:manage` | 用户分页（模糊匹配账号/姓名，可按 `departmentId` / `status` / `roleCode` 筛选） |
| GET | `/user/directory` | 登录 | 用户只读目录（姓名/部门/角色，**不含权限码**） |
...
| PUT | `/user/{userId}` | `user:manage` | 改基本信息（姓名/电话/部门）；`phone` 传空串表示清空，不传表示不修改 |
...
| GET | `/role/{roleId}/permissions` | `user:manage` | 回显该角色已绑定的权限ID（权限抽屉保存前回显用） |
...
| GET | `/department/directory` | 登录 | 部门只读目录（id/编码/名称，不含 remark） |
